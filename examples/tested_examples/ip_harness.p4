...
This harness is for: 
    ...
    - lucid programs should have two exit events: port_out and ip_out
*/
...
control Ingress(
    ...
    action handle_ip_out(){
        ig_tm_md.ucast_egress_port = md.ip_out.egr_port;
        hdr.ip.src_addr = md.ip_out.src;
        hdr.ip.dst_addr = md.ip_out.dst;
    }
    action handle_port_out(){
        ig_tm_md.ucast_egress_port = md.port_out.egr_port;
    }
    action no_op() { }
    table handle_exit_events {
        key = {
            md.dptMeta.exitEventType : exact;
        }
        actions = {
            handle_ip_out;
            handle_port_out;
        ...
        const entries = {
            (e_ip_out) : handle_ip_out();
            (e_port_out) : handle_port_out();
        }
    ...
    apply {
        ...
        // If there's no eventType, don't call the Lucid handlers. 
        if (md.dptMeta.eventType == 0) {
            ciL2Fwd.apply(ig_intr_md, ig_tm_md);                       
        } else {
            // call lucid generated handlers. 
            @DPT_HANDLERS
            // MANUALLY WRITTEN CODE: handle the events ip_out and port_out.
            if (md.dptMeta.exitEventType != 0){
                handle_exit_events.apply();
