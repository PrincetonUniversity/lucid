...
Lucid is a high-level network programming language. It has an event-based programming model inspired by the actor model, a module system inspired by ocaml, and domain-specific constructs that are similar (but higher level) than P4.
...
The frontend also does type inference and focuses on lowering to a core / midend IR that has fewer abstractions. 

The interpreter operates on the core IR and is a full implementation of the lanugage. 
...
The compiler to tofino implements most features, and is fairly well tested. 
...
Compile Lucid with `make` -- this builds the interpreter, tofino compiler, c compiler, and a few other utilities.
...
You can also call it by running `make test` (for interpreter tests) or `make test-cc` (for c compiler tests).
