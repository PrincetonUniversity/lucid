...
let delete_unused_globals ds =
  let id_eq_cid id cid = Cid.equals (Cid.Id id) cid in
  let in_list id cids = CL.exists (id_eq_cid id) cids in
  ...
  let used_global_finder =
    object
      ...
      method! visit_cid _ cid =
        ...
        if b then used_globals := cid :: !used_globals
    end
  in
  ...
  CL.filter_map
    (fun (decl : Syntax.decl) ->
      match decl.d with
      | DGlobal (id, _, _) ->
        if in_list id !used_globals then Some decl else None
      | _ -> Some decl)
    ds
;;
...
let to_single_handler_progs (ds : Syntax.decls) =
  ...
  in
  let (handlers, events), others =
    CL.fold_left handler_split (([], []), []) ds
  in
  CL.map
...
let compile_handlers_to_p4_controls target_filename =
  ...
  let p4_ctl_strs = CL.map compile_single_handler_to_tofino handler_progs in
  p4_ctl_strs
;;
