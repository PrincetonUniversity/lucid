(*  
  Base configuration, available to the frontend and all backends. 
  Each backend should define its own Config module with: 
    - config type 
...
(* the speclist for the Arg module to parse config from 
   named args (all fields except dpt_file) *)
let base_speclist = 
...
(* Use Arg.parse to parse the base speclist plus
   specs from backend-specific config. 
   See InterpConfig.ml for an example. *)
