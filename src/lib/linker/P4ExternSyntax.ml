...
let actions_from_table t = 
  actions_from_rules t.trules
;;
...
let bexpr_to_string b = match b with 
  | Eq(lhs, rhs) ->  lhs^" == "^rhs
  | Neq(lhs, rhs) -> lhs^" != "^rhs
...
let rec stmt_to_string s = match s with 
  ...
  | IfElse (c, s1, s2) -> "if ("^(c)^") { "^(stmt_to_string s1)^"} else { "^(stmt_to_string s2)^"}"
