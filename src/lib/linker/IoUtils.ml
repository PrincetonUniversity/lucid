...
let lucidp4fn_of builddir p4fn =
	builddir^"/"^Consts.p4out_prefix^(Filename.basename p4fn)
;;
  ...
  match (Sys.file_exists dir) with
    | true -> (
      match (Sys.is_directory dir) with 
        | true -> ()
        | false -> error ("tried to create directory "^dir^", but that's already a file.")
    )
