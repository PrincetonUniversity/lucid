...
type code_stmt = 
  ...
  | LExpr of code_expr 
  | LStmt of code_expr
  | LBlock of {lb_label : code_expr; lb_body : code_stmt;} (* label * body *)
  | LPragma of {lb_name : code_expr; lb_exprs : code_expr;}
...
let to_lblock_direct label_exp body_st = LBlock {lb_label = label_exp; lb_body = body_st;}
...
let to_pragma pname pbody = LPragma {lb_name = to_code_expr pname; lb_exprs = to_code_expr pbody;}
let to_pragma_name pname = LPragma {lb_name = to_code_expr pname; lb_exprs = to_code_expr "";}
...
let rec string_of_code_stmt ls = 
  match ls with 
    ...
    | LBlock {lb_label = lbl; lb_body = bdy;} -> (string_of_code_str lbl)^"{"^(string_of_code_stmt bdy)^"}"
    | LPragma {lb_name = lbl; lb_exprs = bdy;} -> "@"^(string_of_code_str lbl)^"("^(string_of_code_str bdy)^")"
...
;;
let tab () = pp_open_vbox stf 4;    ()
let untab () = pp_close_box stf (); ()
;;
...
let rec dbg_pprint_code_stmt ls = 
  match ls with 
    | LNoop cs -> fprintf stf "Noop [[%s]]@," (String.trim (string_of_code_str cs))
    | LAdded cs -> fprintf stf "Added [[%s]]@," (String.trim (string_of_code_str cs))
    | LExpr cs -> fprintf stf "Expr [[%s]]@," (String.trim (string_of_code_str cs))
    | LStmt ls -> fprintf stf "Stmt [[%s;]]@," (String.trim (string_of_code_str ls))
    ...
    | LPragma {lb_name = lbl; lb_exprs = bdy;} -> 
      fprintf stf "Pragma [[%s]](%s)@," (String.trim (string_of_code_str lbl)) (String.trim (string_of_code_str bdy))
    | LSeq lss -> (
      tab ();
      fprintf stf "Seq [[@,";
...
let is_whitespace c = match c with 
  ...
  | _ -> false
;;
let is_not_stmt_delim c = match c with 
  | '}' | '{' | ';' | '@' -> false
  | _ -> true
;;
let is_not_pragma_delim c = match c with 
  | '}' | '{' | ';' | ')' | '(' -> false
  | _ -> true
;;
let is_end_of_pragma_name c = 
    is_whitespace c 
    || (not (is_not_pragma_delim c))
;;
...
(* <statement:code_expr>; *)
let statement = (take_while is_not_stmt_delim) <* (A.char ';') 
  >>| fun codestr -> (
...
let pragma_with_params = A.string "@" *> 
  (
    (take_till is_end_of_pragma_name) <* (A.char '(')
    >>= (fun pname_str -> 
          (take_while is_not_pragma_delim) <* (A.char ')')
          >>| (fun pbdy_str -> to_pragma (pname_str) (pbdy_str))
        )
  )
;;
(* @<name:code_expr> *)
let name_pragma = A.string "@" *> 
...
let pragma = whitespace >>= (fun ws -> 
  raw_pragma >>| (fun lprag -> to_lseq [(to_lexpr ws);lprag])
)
;;
...
let basic_statement = line_comment <|> line_include <|> line_define <|> multiline_comment <|> pragma <|> statement
;;
...
let block = 
  fix ( fun block -> 
    ...
    let block_body = parse2_map 
      ((A.char '{') *> A.many (basic_statement <|> block)) (* body *)
      (whitespace_lstmt<* A.string "}" >>| (fun v -> [v])) (* terminating whitespace *)
...
let prog = whitespace *> (A.many (basic_statement <|> block) >>| to_lseq) <* whitespace
;;
...
let parse_prog progstr = 
  ...
  match res with 
    | Ok ls -> ls 
    | Error msg -> printf "error!\n"; failwith msg
;;


(*** tokenize code in parts of the tree ***)

module Tok = struct 
  let is_indent_start c = match c with 
    | '_' | 'A'..'Z' | 'a'..'z' -> true | _ -> false
  let is_indent c = match c with 
    | '_' | 'A'..'Z' | 'a'..'z' | '0' .. '9' -> true | _ -> false
  let is_other c =
        not (is_indent_start c) 
    &&  not (is_indent c) 
  ;;
  type p4token = 
    | Ident       of string
  ...
  let p_Indent = parse2_map
    (take_while1 is_indent_start)
    (take_while is_indent)
    (fun s1 s2 -> s1^s2) >>| fun s -> Ident s
  let p_Other      = take_while1 is_other  >>| fun s -> Other s
  ;;

  let p_p4tokens = A.many (p_Indent <|> p_Other) <* A.end_of_input
  ;;

  let tokenize_codeexpr ce = 
    let res = Angstrom.parse_string ~consume:Consume.All p_p4tokens ce.codestr in 
    match res with 
      | Ok ts -> ts 
      | Error msg -> printf "error tokenizing string!\n"; failwith msg
  ;;
  let string_of_p4token t = match t with 
    | Ident s | Other s -> s
  ;;
  ...
  match cs with 
    | LPragma {lb_name = lbl; _} -> (
      match (code_expr_equals lbl {codestr=prag_name;}) with 
        | true -> to_added ("//Compiler-generated code for: @"^prag_name^"\n"^new_str)
        | false -> cs
    )
    | LBlock brec ->
...
(*           print_endline (sprintf "[find_and_transform] found: %s in\n---\n%s\n---\n" bname (sigstring_of_block cs)); *)
          (tfun cs)
        | false -> LBlock {brec with lb_body =(find_and_transform bname tfun brec.lb_body)} (* recurse on block body *)
    )
    | LSeq lss -> (
...
let rec fold_split_f
  ...
  let non_match_outp () =  match prag_cs with 
  ...
  match cs with 
    ...
    | LSeq inner_css -> (* step into a sequence *)
...
let igr_condition_pre_and_post_dispatch igr_apply_cs = 
  let (igr_apply_lbl, igr_apply_stmts) = match igr_apply_cs with 
    | LBlock {lb_label = lb_label; lb_body = cs;} -> (
        match cs with 
        | LSeq css -> (lb_label, css)
        | _ -> (lb_label, [cs])
      )
    | _ -> error "[transform_pre_dpt_ingress] ingress apply block... is not a block"
  in 
  linker_report (sprintf "transforming existing P4 ingress.apply to skip background event packets...");
  (* split it at @DPT_CALLS *)
  let (pre_css, call_cs, post_css) = split_by_prag Consts.dpt_igr_call igr_apply_stmts in 
  (* wrap the pre_block with the if statement *)
  let pre_label = to_code_expr 
    (sprintf "\n\t//Linker-added condition: don't apply pre-dispatch tables to background event packets.\n\tif (%s == 0)" (Consts.dpa_global_name^"."^Consts.branchArg_name)) 
  in 
  let pre_css_new = [encap_in_block pre_label pre_css] in 
  let post_label = to_code_expr 
    (sprintf "\n\t//Linker-added condition: don't apply post-dispatch tables to packets without exit events.\n\tif (%s != 0)" (Consts.dpa_global_name^"."^Consts.exitEventFldName)) 
  in 
...
let ingress_bg_event_trans cs = 
  ...
  let igr_tf igr_cs = 
    ...
    let new_igr_cs = find_and_transform "apply" igr_condition_pre_and_post_dispatch igr_cs in 
    new_igr_cs
  in 
...
let pragma_replace_trans cs pragma_obj_dict = 
  let fold_f p4_tree (pragname, newcode) = 
    ...
    !dprint_endline ("[pragma_replace_trans] -------------- block: "^pragname^ "--------------");
    ...
    match (p4_tree <> new_p4_tree) with 
      | true -> new_p4_tree
      | false -> Console.warning (sprintf "did not replace DPT pragma %s in P4" pragname); new_p4_tree
  in 
...
let link_p4 obj_dict p4fn = 
  ...
  let p4_parse_tree = parse_prog p4prog in
  (* transform the ingress function so that the code before and after the 
  call to DPT dispatch doesn't apply to event packets. *)
  let p4_parse_tree = ingress_bg_event_trans p4_parse_tree in 
  (* replace all the DPT pragmas with compiler generated P4 code *)
  let p4_parse_tree = pragma_replace_trans p4_parse_tree obj_dict in 
  (string_of_code_stmt p4_parse_tree)
;;
