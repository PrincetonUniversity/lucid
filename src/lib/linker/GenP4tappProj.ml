...
let templates_dir =
  Filename.dirname Sys.argv.(0) ^ "/tofinoAssets/p4tappTemplates"
;;

let p4tjson_basename = "p4tapp.json"
let p4tappfn_of dir base = dir ^ "/" ^ base ^ ".p4tapp"

let p4tgen_report str =
  Console.show_message str ANSITerminal.Green "P4tapp generator"
;;
...
let update_p4tapp_config json_fn local_p4fn =
  p4tgen_report
    (sprintf
       "updating p4tapp config json in %s with P4 src name: %s"
       json_fn
       local_p4fn);
...
let generate p4_str p4fn dptfn builddir p4tappfn =
  ...
  let full_p4lucid_dst =
    lucidp4fn_of p4tapp_p4_srcdir (Filename.basename p4fn)
  in
