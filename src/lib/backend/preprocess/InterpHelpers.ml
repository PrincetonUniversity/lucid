...
let eval_vint_ty i ty = 
  value_to_exp (vint_ty i ty)
;;
...
let raw_ty_of_exp exp = 
  TyTQVar.strip_links (ty_of_exp exp).raw_ty
;;

let intwidth_of_exp (exp : exp) : int =
  match exp.ety with
  | None -> error "untyped expression after type checking/inference..."
  | Some rty -> intwidth_from_raw_ty rty
;;
...
let is_atomic exp =
  ...
  | EProj _ | ERecord _ ->
    error "records and projection operators should be removed before IR."
;;

let is_bool_non_immediate exp = 
  (is_bool exp) && (not (is_immediate exp))
;;
...
let replace_assign_rhs stmt exp = 
  match stmt.s with 
    ...
    | _ -> error "tried to replace the rhs of a statement that is not an assign/local"
;;
...
(**** inlining helpers ****)
(* replace every expression that references t with expression n *)
