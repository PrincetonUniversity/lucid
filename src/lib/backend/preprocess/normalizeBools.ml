...
module NormalizeRelops = struct
  ...
  and normalize_erelop (op : op) (args : exp list) ety espan =
    ...
    | Eq | Neq -> (
      ...
      | [e1; e2] ->
        let test_var_cid, calc_test_var = create_eq_precompute_test e1 e2 in
        let new_e1 = aexp (EVar test_var_cid) e1.ety Span.default in
        let new_e2 = value_to_exp (vint 0 32) in
      ...
      | _ -> error "unexpected form for eq eop")
    | Less | More ->
      error
        "inequalities are not supported -- TODO once sat sub is implemented "
  ...
  *)
  ...
  let rec z3_from_expr ctx exp =
    ...
    | EOp (op, args), _ ->
      ...
      | Less | More ->
        error
          "got to an less or greater than in z3_from_expr -- this probably means that \
           the clause transformation pass has not run, or went wrong. "
      | Cast _ ->
    ...
    | EVar _, Some rty ->
      ...
      let tystr = Printing.raw_ty_to_string rty in
      sprintf
    ...
    | ERecord _, _ -> error "z3_from_expr got a record expression."
  ;;
  ...
  let rec exp_of_z3e z3e =
    ...
    | false ->
      ...
      | _ -> fold_commutative_eop (eop_ty op arg_exps rty))
  ;;
  ...
  let dnf_of_if if_stmt =
    ...
    !dprint_endline (sprintf "old expression:  %s\n" (Printing.exp_to_string exp));
    !dprint_endline (sprintf "new expression:  %s\n" (Printing.exp_to_string new_exp));
    !dprint_endline (sprintf "new expression show: %s\n" (Printing.exp_to_string new_exp));
