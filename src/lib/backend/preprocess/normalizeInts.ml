...
let dprint_eop exp = 
    ...
    CL.iter (fun exp -> !dprint_endline ("[balance_assign_exp]"^(Printing.exp_to_string exp))) flat_args;
    !dprint_endline "---[balance_assign_exp] flat args--- "
...
let balance_assign_exp s = 
    ...
    let new_exp = match op_of_exp exp with 
      (* plus is commutative *)
      | Some Plus -> 
        dprint_eop exp;
        let new_exp = balance_eop_tree exp in 
        !dprint_endline ("[balance_assign_exp] original expression:\n"^(Printing.exp_to_string exp));
        !dprint_endline ("[balance_assign_exp] balanced expression:\n"^(Printing.exp_to_string new_exp));
        new_exp
      | Some _ -> exp
      | None -> exp 
    in 
...
and to_atomic exp = 
  match (is_atomic exp) with 
    ...
    | false ->     
      ...
      let immediate_args, precompute_stmts = CL.map to_immediate args |> CL.split in 
    ...
    object
      ...
      method! visit_statement ctx stmt =       
        match (stmt.s) with 
          | SAssign _ | SLocal _ ->
            precompute_stmts <- [];
            (* recurse on statement tree *)
            let transformed_stmt = super#visit_statement ctx stmt in 
       ...
       method !visit_exp in_assign exp = 
        ...
        match in_assign with 
          | true -> 
            ...
            !dprint_endline ("[transform_precompute_exps.visit_exp] exp: "^(Printing.exp_to_string exp));
            let atomic_exp, new_precompute_stmts  = to_atomic exp in 
            precompute_stmts <- precompute_stmts @ new_precompute_stmts;
            !dprint_endline ("[transform_precompute_exps.visit_exp] atomic exp: "^(Printing.exp_to_string atomic_exp));
            atomic_exp
          | false -> 
            exp 
