      ...
      match (rhs_of_stmt statement.s) with 
        | Some exp -> (
          match (is_bool_non_immediate exp) with 
            | true  ->
              trans_info ("[to_bool_immediate_rhs] eliminating non-immediate boolean assignment in: "^(Printing.stmt_to_string statement));
...
let eliminate_bool_values_and_types ds = 
  let v = object (_)
    ...
    method! visit_DMemop _ id body = 
      DMemop(id, body)
    method !visit_VBool _ b = 
      VInt(zint_of_bool b)
    method !visit_TBool _ = 
      TInt (IConst bool_sz)
    end 
  in
  v#visit_decls (statement SNoop) ds
...
let do_passes (ds:decls) : Syntax.decls =
  ...
  let ds = eliminate_bool_values_and_types ds in 
  log_prog "after phase 3: immediate elimination" ds;
  trans_info "boolean elimination passes complete";
