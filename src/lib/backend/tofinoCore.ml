...
and main_handler =
  { main_id : id
  ; hdl_selector : id * ty
      (* the variable containing the current event's integer identifier. note: this should be a pervasive.*)
  ; hdl_enum : (id * int) list
      (* integer identifiers for each handler / event *)
  ; hdl_params : (id * params) list (* parameters of each handler *)
  ; shared_locals : (id * ty) list
      (* local variables that can be accessed from any event *)
  ; default_hdl : id option
      (* the handler to execute for packets that arrive on non-lucid ports *)
  ; main_body : statement list
      (* the body of the main handler.
...
and event_output =
  ...
  ; (* all possible sequences of events
    ...
    (* all the generate sequences that may appear in this program. *)
  ; generate_sequences : statement list list
    }
...
let find_generated_event_id_sequences stmt =
  find_statement_paths
    []
    (fun stmt ->
      match stmt.s with
...
(* find all the sequences of generate statements that occur in the program. 
   These will be used later to produce a mapping from generated events 
   to header fields. *)
...
let test_gen_sequencer () =
  ...
  in
  let _ = find_generated_event_id_sequences stmt in
  ()
;;
  ...
  let main_body =
    let handler_branches branches dec =
      ...
      | TDHandler (hdl_id, _, (_, stmt)) ->
        let hdl_num =
          match List.assoc_opt hdl_id hdl_enum with
          | None ->
            error
              "[generate_merged_handler] could not find handler id in enum. Do \
               events and handlers have the same internal IDs?"
    ...
    in
    let ehdl_selector =
      var_sp (Cid.id (fst hdl_selector)) (snd hdl_selector) Span.default
    in
    ...
    | hd :: tl ->
      (match hd with
       | { td = TDHandler (i, s, (p, _)) } ->
         { hd with td = TDHandler (i, s, (p, snoop)) }
         :: erase_handler_bodies tl
       | _ -> hd :: erase_handler_bodies tl)
  in
  let event_output =
    { recirc_mcid_var = Id.create "recirc_mcid", ty (TInt 16)
    ; ev_gen_seqs =
    find_generated_event_id_sequences (List.hd main_body) |> MiscUtils.unique_list_of
    ; generate_sequences = find_generate_sequences (List.hd main_body) (* don't take unique *)
    }
  in
  let tds =
    erase_handler_bodies decls
    @ [ { td =
            TDMain
              { main_id
              ; hdl_selector
...
let add_lib_handler decls =
  ...
  let event_output =
    { recirc_mcid_var = Id.create "recirc_mcid", ty (TInt 16)
    ; ev_gen_seqs =
    find_generated_event_id_sequences main_body |> MiscUtils.unique_list_of
    ; generate_sequences = find_generate_sequences main_body (* don't take unique *)
    }
  in
  let tds =
    erase_handler_bodies decls
...
let update_main ds new_main_d =
  List.map
    (fun dec ->
      match dec.td with
      | TDMain _ -> { dec with td = TDMain new_main_d }
      | _ -> dec)
    ds
;;
...
let add_shared_local ds tmp_id tmp_ty =
  ...
  let new_main =
    { old_main with shared_locals = (tmp_id, tmp_ty) :: old_main.shared_locals }
  in
...
let decl_of_tdecl tdecl =
  ...
  | TDMemop m ->
    { d = DMemop m; dspan = tdecl.tdspan; dpragma = tdecl.tdpragma }
  | TDExtern (i, t) ->
    { d = DExtern (i, t); dspan = tdecl.tdspan; dpragma = tdecl.tdpragma }
  | TDAction a ->
    { d = DAction a; dspan = tdecl.tdspan; dpragma = tdecl.tdpragma }
...
let main_to_string mainsig =
  ...
  ^ (List.map
       (fun (id, ty) ->
         CorePrinting.ty_to_string ty ^ " " ^ CorePrinting.id_to_string id ^ ";")
       mainsig.shared_locals
    |> String.concat "\n")
  ^ "\n// MAIN HANDLER \n"
  ^ "handler main(...){\n"
  ^ ((List.mapi (fun i stg_stmt ->
        "// Stage "
        ^ string_of_int i
        ^ "\n"
...
let tdecl_to_string tdec =
  match tdec.td with
  | TDMain mainsig -> main_to_string mainsig
...
let tdecls_to_string tdecs =
  List.map tdecl_to_string tdecs |> String.concat "\n"
;;
