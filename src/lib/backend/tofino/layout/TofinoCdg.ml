...
let print_node_in_edges lbl g v =
    !dprint_endline (lbl^"vertex:"^(TofinoCfg.summarystr_of_stmt v.stmt true));
    !dprint_endline (lbl^"in edges:");
    Cfg.iter_pred 
        (fun p ->   
            let (s, e, t) = Cfg.find_edge g p v in
            !dprint_endline (lbl^"SRC:"^(TofinoCfg.summarystr_of_stmt s.stmt true));
            !dprint_endline (lbl^"DST:"^(TofinoCfg.summarystr_of_stmt t.stmt true));
            !dprint_endline (lbl^"in edge:"^(TofinoCfg.str_of_edge_condition e))
        )
        g
        v;
;;
...
let rec precondition_of_vertex orig_v idom g v =
    ...
    | [(_, e, _)] -> 
        ...
        !dprint_endline (TofinoCfg.summarystr_of_stmt orig_v.stmt true);
    ...
    | _ -> 
        let d = idom v in 
        !dprint_endline@@"[precondition_of_vertex] finding precondition for vertex with multiple in edges:"^(TofinoCfg.summarystr_of_stmt orig_v.stmt true);
        print_node_in_edges "" g v;
        !dprint_endline@@"[precondition_of_vertex] recursing FROM:"^(TofinoCfg.summarystr_of_stmt v.stmt true);
        !dprint_endline@@"[precondition_of_vertex] recursing TO:"^(TofinoCfg.summarystr_of_stmt d.stmt true);
    ...
    (match pc with
        | CMatch(match_cond) -> (
            ...
            | None -> 
                ...
                error (TofinoCfg.str_of_edge_condition pc)
        ...
        let update_for_outedge (s, e, d) g =
            let new_e = match (pc, e) with
                ...
                | (CExp(_), _) | (_, CExp(_)) -> 
                    ...
                    print_endline (TofinoCfg.str_of_vertex v);
                    error "CExp constraints not implemented. Convert to match"
            in 
            !dprint_endline ("successor node: "^(TofinoCfg.summarystr_of_stmt d.stmt true));
...
(*                 (* craft the match statement *)
            ...
            | _, _ -> ( 
                ...
                | CMatch(match_cond) -> 
                    ...
                    !dprint_endline ("ORIGINAL VERTEX:"^(TofinoCfg.summarystr_of_stmt v.stmt true));
                    print_node_in_edges "[NODE IN EDGES]" g v;
                    !dprint_endline ("INPUT PC:"^(TofinoCfg.str_of_edge_condition pc));
                    !dprint_endline ("NORMALIZED VERTEX:"^(TofinoCfg.summarystr_of_stmt new_stmt true));
                    {v with stmt = new_stmt;}
                ...
                | CNone -> 
                    ...
                    !dprint_endline ("NORMALIZED NO CONDITION VERTEX:"^(TofinoCfg.summarystr_of_stmt new_stmt true));
                    {v with stmt = new_stmt;}
