...
let get_dependencies dconstr s_var_getter t_checker (s:vertex_stmt) (ts:vertex_stmt list) =   
  ...
  !dprint_endline@@"s:"^(TofinoCfg.summarystr_of_stmt s.stmt true);
  !dprint_endline@@"ts:";
  List.iter (fun t -> !dprint_endline@@"DEPENDEE:"^(TofinoCfg.summarystr_of_stmt t.stmt true)) dependent_ts; 
  !dprint_endline "---------";
  let res = CL.map
