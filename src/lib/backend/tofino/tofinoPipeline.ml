...
let print_if_debug str = if Config.base_cfg.debug then Console.report str
...
let core_passes ds portspec = 
  ...
  let ds = if Config.base_cfg.partial_interp
    then (
...
let compile ds portspec =  
  ...
  let py_eventlib =
    if 
      ...
      TofinoConfig.cfg.serverlib then PyEventLib.coresyntax_to_pyeventlib ds else ""
  in
  ...
  let py_ctl = ControlPrinter.pyctl_of_prog tofino_prog TofinoConfig.cfg.ctl_fn in
