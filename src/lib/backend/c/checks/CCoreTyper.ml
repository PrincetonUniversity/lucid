...
;;
let ty_err_in_decl decl_opt str = 
  match decl_opt with
  | None -> 
    error str
  | Some decl -> error @@ 
  ...
  method! visit_ty () ty = 
    ...
    | TTuple tys -> 
      ...
      if (total_size mod 8 <> 0) then 
        ty_err_in_decl cur_decl_opt @@"record, union, or tuple >>>"^(CCorePPrint.ty_to_string ty)^" <<< has a total size that is not a multiple of 8 bits"
    | _ -> ()
end
;;
