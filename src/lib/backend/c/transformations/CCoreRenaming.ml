...
(* Make events, handlers, and patterns have the same ids.
  ...
  let ev_id_map = List.map (fun inp -> 
      ...
      (Cid.name (hdl_cid), (hdl_cid))) 
    handler_defs 
  in
  let param_id_map = List.fold_left 
    (fun acc (hdl_cid, _, params, _) -> 
      let param_id_map = List.map (fun (pid, _) -> (Cid.name pid, pid)) params in
      (hdl_cid, param_id_map)::acc)
    []
    handler_defs
  in
  let rename id = 
    match List.assoc_opt (Cid.name id) ev_id_map with 
    | Some(id) -> id
    | None -> id
  in
  ...
  let rename_params evid params = 
    ...
    List.map 
      (fun (param_id, ty) -> 
        match List.assoc_opt (Cid.name param_id) str_to_id with 
  ...
  let v = object (_) inherit [_] s_map as super 
    ...
    method! visit_exp () e = 
      ...
      match e.e with
        | EVar(cid) when is_tevent e.ety ->
          {e with e=EVar(rename cid)}
        | _ -> e

    method! visit_PEvent () event_id params = 
      let params = super#visit_params () params in
      let event_id = rename event_id in
