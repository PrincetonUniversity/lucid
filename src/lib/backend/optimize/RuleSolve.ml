...
let string_of_match (m:pattern) : string = 
	CL.map 
	(fun (m_var, m_val) -> 
		sprintf "%s = %s" (P4tPrint.str_of_varid m_var) 
		(match m_val with | Exact i -> Int.to_string (Integer.to_int i) | Any -> "*")
	)
	m 
	|> Caml.String.concat ","
;;
let string_of_rule (r:rule) = 
	match r with 
		| Match(r_id, m_exp, a_id) ->
			sprintf "%s: (%s) -> %s" (P4tPrint.str_of_private_oid r_id) (string_of_match m_exp) (P4tPrint.str_of_private_oid a_id)
		| OffPath m_exp -> 
			sprintf "    (%s) -> %s" (string_of_match m_exp) "OFFPATH"


let string_of_rules t : string = 
	Caml.String.concat "\n" (CL.map 
		(fun r -> sprintf "%s" (string_of_rule r)) t)
;;
...
let eqn_of_ordered_rule ctx (rules: rule list) (rule:rule) = 
	let matched_fst_rules, _ = BatList.split_at (Caml.Option.get (BatList.index_of rule rules)) rules in 
	let (ctx, neg_eqns) = CL.fold_left 
...
let is_rule_tuple_feasible  (s:rule list) (t:rule list) (s_rule:rule) (t_rule:rule) = 
...
let find_feasible_paths (s:rule list) (t:rule list) : ((rule * rule) * bool) list = 
	...
	let check_pair_by_name (s_rule, t_rule) = 
		...
		printf "\t%s [position %i] --> %s [position %i] feasible? %s\n" (string_of_rule s_rule) s_idx (string_of_rule t_rule) t_idx (Bool.to_string feas);
		feas	
	in 
