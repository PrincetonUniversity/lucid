...
let log_rules rules = CL.iter (fun r -> DBG.printf outc "%s\n" (dbgstr_of_rule r)) rules ;;
...
let intersect_conditions (a_cond:condition) (b_cond:condition) : condition option = 
	match a_cond with 
...
let intersect_patterns (a_pat:pattern) (b_pat:pattern) : pattern option = 
	...
	let ab_conds = CL.map (fun (a_cond, b_cond) -> intersect_conditions a_cond b_cond) ab_conds in 
	let has_intersect = CL.fold_left (fun intersect_exists cond_opt -> match cond_opt with | None -> false |Some _ -> intersect_exists) true ab_conds in 
...
let gen_intersect_rule s_rule t_rule union_aid = 
	...
	let intersect_rule = match union_aid with 
...
let par_gen_union_acn_request_custom name_fcn s_rule t_rule = 
	...
	match (is_offpath s_rule) with 
		...
		| false -> (
			...
			| false -> let new_aid = (name_fcn s_aid t_aid) in Some new_aid, Parallel(Some new_aid, s_rule, t_rule)
		)
;;
...
let gen_union_acn t_tid cid_decls new_acn_request = 
	let (new_acn_id, s_rule, t_rule) = match new_acn_request with
		...
		| Some new_acn_id -> (
			...
			| false -> (
				(if ((is_offpath s_rule) && (is_offpath t_rule)) 
					then (error "Trying to generate a union action from two offpath rules. This should not happen. ")
				);
				let s_aid = aid_of_rule s_rule in 
				let t_aid = aid_of_rule t_rule in 
				...
				let next_tids = match new_acn_request with
				...
				let next_tids = CL.filter (fun next_tid -> not (Cid.equals t_tid next_tid)) next_tids in 
				DBG.printf outc "[gen_union_acn] new_acn_id=%s s_aid=%s t_aid=%s\n" (P4tPrint.str_of_private_oid new_acn_id) (P4tPrint.str_of_private_oid s_aid) (P4tPrint.str_of_private_oid t_aid);
				let new_acn = Action (
					new_acn_id, 
					unique_list_of (s_oids@t_oids), 
					next_tids
					)
				in 
...
let merge_t_into_s gen_union_acn_request st_feas_checker t_rule (new_action_requests, t_derived_s_rules, new_s_rules_so_far) s_rule = 
	DBG.printf outc "[merge_t_into_s] attempting to merge\n[merge_t_into_s] s_rule: %s\n[merge_t_into_s] t_rule: %s \n" (dbgstr_of_rule s_rule) (dbgstr_of_rule t_rule);
	...
	let (more_new_action_requests, more_t_derived_s_rules, more_new_s_rules) = 
		...
		| true -> ([], [], [s_rule]) (* s is derived from a higher priority rule in t (which is mutually exclusive with r_rule) *) 
		| false -> (
			let st_feas = st_feas_checker s_rule t_rule in (* s and t for parallel tables, s then t for sequential tables *)
			match (st_feas) with
			...
			| true -> (
				...
				let s_is_reachable_after_s_t = RS.is_r_still_feasible s_rule [s_rule; t_rule] in (* QUESTION: do we also need to check whether s is feasible with respect to other new_s_rules_so_far? *)
				match (s_is_reachable_after_s_t) with 
				| false -> ( (* s_and_t covers s, so we want to add only s_and_t, which is derived from t. *)
					DBG.printf outc "\t[merge_t_into_s]deleting: %s\n" (dbgstr_of_rule s_rule); 
					DBG.printf outc "\t[merge_t_into_s]adding: %s\n" (dbgstr_of_rule intersect_rule); 
					([gen_acn_req], [intersect_rule], [intersect_rule])
				)
				| true -> ( (* s_and_t does not cover s, so we want to add s_and_t, then s. *)
					DBG.printf outc "\t[merge_t_into_s]adding: %s\n" (dbgstr_of_rule intersect_rule); 
					DBG.printf outc "\t[merge_t_into_s]adding: %s\n" (dbgstr_of_rule s_rule); 
	...
	let out_tup = new_action_requests@more_new_action_requests, t_derived_s_rules@more_t_derived_s_rules, new_s_rules_so_far@more_new_s_rules in 
	out_tup	
;;


(* merge a rule from t into all the rules in s *)
let merge_t_into_all_s gen_union_acn_request st_feas_checker t_only_feas_checker (new_action_requests, t_derived_s_rules, s_rules) t_rule = 
	...
	log_rules s_rules;
	DBG.printf outc "\n--------\n";
	let new_action_requests, updated_t_derived_s_rules, updated_s_rules = CL.fold_left (merge_t_into_s gen_union_acn_request st_feas_checker t_rule) (new_action_requests, t_derived_s_rules, []) s_rules in 
	(* if we can reach t without going through a rule in s, we need to add t to the end of the new rule set. *) 
	let updated_t_derived_s_rules, updated_s_rules = match (t_only_feas_checker updated_s_rules t_rule) with 
	...
	let updated_s_rules = CL.filter (RS.is_reachable_in_order updated_s_rules) updated_s_rules in 
	let num_reachable_rules = CL.length updated_s_rules in 
	DBG.printf outc "[merge_t_into_all_s] filtered out %i unreachable rules\n" (num_rules - num_reachable_rules);
...
let merge_all_t_into_all_s gen_union_acn_request st_feas_checker t_only_feas_checker s_rules t_rules  = 
	let new_action_requests, all_t_derived_s_rules, final_s_rules = CL.fold_left (merge_t_into_all_s gen_union_acn_request st_feas_checker t_only_feas_checker) ([], [], s_rules) t_rules in 
	let _ = all_t_derived_s_rules in (* don't need to return this *)
...
let merge_tables gen_union_acn_request check_st_feas check_t_only_feas cid_decls s_tid t_tid = 
	...
	let merged_s_rules, new_action_requests = (merge_all_t_into_all_s gen_union_acn_request (check_st_feas s_acns_map t) check_t_only_feas) s_rules t_rules in 


	(* generate new actions and add them to cid decls*)
	let cid_decls = CL.fold_left (gen_union_acn t_id) cid_decls (unique_list_of new_action_requests) in 

	(* generate s with new rules *)
	let new_s = Table(s_id, merged_s_rules, s_stage) in 

	DBG.printf outc "[merge_tables] finished merging into table %s -- new table has %i rules\n" (P4tPrint.str_of_private_oid s_id) (CL.length merged_s_rules);
	...
	let unused_in_st = CL.filter (fun orig_aid -> (not (CL.mem orig_aid new_st_aids))) (orig_t_aids@orig_s_aids) in 
	(*[packetin~0, merged_acn~1] *)

	let cid_decls = CL.fold_left (fun cid_decls unused_aid -> Cid.remove cid_decls unused_aid) cid_decls unused_in_st in 
...
let seq_t_only_feas st_rules t_rule = let _, _ = st_rules, t_rule in false
...
let seq_st_feas s_acns_map t s_rule t_rule = 
	match s_rule with 
	| OffPath _ -> false (* if s is offpath, then there's no way we can ever get to t. *)
	| Match(_, _, s_acn_id) -> (
		DBG.printf outc "seq_st_feas: s_acn_id = %s\n" (P4tPrint.str_of_private_oid s_acn_id);
...
let parallel_merge (cid_decls:declsMap) s_tid t_tid = 
	DBG.printf outc "---PARALLEL MERGE: tables %s and %s---\n" (P4tPrint.str_of_private_oid s_tid) (P4tPrint.str_of_private_oid t_tid);
	merge_tables par_gen_union_acn_request par_st_feas par_t_only_feas cid_decls s_tid t_tid
;;

(* merge two sequential tables, return updated cid_decls with new s_tid *)
let sequential_merge (cid_decls:declsMap) s_tid t_tid = 
	DBG.printf outc "---SEQUENTIAL MERGE: tables %s and %s---\n" (P4tPrint.str_of_private_oid s_tid) (P4tPrint.str_of_private_oid t_tid);
	merge_tables seq_gen_union_acn_request seq_st_feas seq_t_only_feas cid_decls s_tid t_tid
;;
...
let null_action_copy_of_s_tid cid_decls s_tid t_tid = 
	...
	match s with 
		| Table (tbl_id, rules, stage_opt) -> (
			...
			let new_rules, new_null_actions_lists = CL.split (CL.map (null_action_copy_of_rule cid_decls t_tid) rules) in 
		...
		| _ -> error (sprintf "couldn't find table: %s\n" (P4tPrint.str_of_private_oid s_tid))
;;
...
let propagate_condition (cid_decls : declsMap) s_tid t_tid = 
	DBG.printf outc "[propagate conditions] table %s to table %s---\n" (P4tPrint.str_of_private_oid s_tid) (P4tPrint.str_of_private_oid t_tid);

	(* make a copy of the data structure, with table s_tid having actions replaced by null actions *)	
	let (noop_s_cid_decls, noop_s_tid) = null_action_copy_of_s_tid cid_decls s_tid t_tid in 
...
type propagation_type = 
	...
	| OneMustMatch (* table t executes if any of its conditions hold. *)
;;
let propagate_condition_generic prop_type cid_decls s_tid t_tid = 
	DBG.printf outc "[propagate_condition_generic] table %s to table %s\n" (P4tPrint.str_of_private_oid s_tid) (P4tPrint.str_of_private_oid t_tid);

	(* make a copy of the data structure, with table s_tid having actions replaced by null actions *)	
	let (noop_s_cid_decls, noop_s_tid) = null_action_copy_of_s_tid cid_decls s_tid t_tid in 
	(* merge the tables into st_tid *)	
	let merge_name_fn = (fun _ b -> b) in 
	(* let merge_name_fn = Cid.concat in  *)
	let noop_s_cid_decls = match prop_type with 
		| AllMustMatch -> DBG.printf outc "[propagate_condition_generic] AllMustMatch\n"; 
		(*sequential merge, with different union acton name function*) 
		merge_tables (seq_gen_union_acn_request_custom merge_name_fn) seq_st_feas seq_t_only_feas noop_s_cid_decls noop_s_tid t_tid
		| OneMustMatch -> DBG.printf outc "[propagate_condition_generic] OneMustMatch\n"; 
		(*parallel merge, with different union action name function*)  
		merge_tables (par_gen_union_acn_request_custom merge_name_fn) par_st_feas par_t_only_feas noop_s_cid_decls noop_s_tid t_tid
	in 
	(* get the st table *) 
	let noop_st = Cid.lookup noop_s_cid_decls noop_s_tid in 
	(* remove the st' table, add the t table back in. The s table should still be there, because we only operated on a copy. *)
...
let merge_pred_conditions cid_decls s_tids t_tid = 
	...
	let fold_f t_tid (cid_decls, new_s_tids) s_tid = 
		let cid_decls, new_s_tid = null_action_copy_of_s_tid cid_decls s_tid t_tid in 
		(cid_decls, new_s_tids@[new_s_tid])
	in 
	let cid_decls, s_copy_tids = CL.fold_left (fold_f t_tid) (cid_decls, []) s_tids in 
	...
	let merge_pred_pair cid_decls a_tid b_tid = merge_tables 
	...
	in 
	let cid_decls = CL.fold_left (fold_f merged_preds_tid) cid_decls s_copy_tids in 

	(* 3. propagate conditions from merged_preds_tid to t_tid *)
	let cid_decls = propagate_condition_generic AllMustMatch cid_decls merged_preds_tid t_tid in 
	(* 4. get the new t_tid *)
