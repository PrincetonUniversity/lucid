...
instructionSyntax objects. Used for optimizations. *)
...
let get_edges (cid_decls:declsMap) : (cid*cid) list = 
  ...
  let decl_to_out_edges cid_decls node_id = 
    ...
    | Action (_, obj_ids, next_tids) -> nextid_to_edges node_id (next_tids@obj_ids) (* action has pointers to tables and objects *)
    | Table (_ ,rules, _) -> nextid_to_edges node_id (rules_to_aids rules)
    | SInstrVec (_, reg_id, _, _, _, _) -> nextid_to_edges node_id [reg_id]
    (* Hashers and InstrVecs aren't here because they don't have any out edges. *)
    | _ -> []
  in   
  let x = (fun edge_acc d -> (decl_to_out_edges cid_decls d)@edge_acc) in 
 ...
 *)
(* type check ids *)
let is_tbl cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | Table _ -> true
  | _ -> false
;;
let is_reg cid_decls oid = match (Cid.lookup cid_decls oid) with
  | RegVec _ -> true
  | _ -> false
;;
let is_callable cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | Table _ -> true
  | SchedBlock _ -> true
  | _ -> false
;;
let is_acn cid_decls oid = match (Cid.lookup cid_decls oid) with
  | Action _ -> true
  | _ -> false
;;

let is_meta cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | GlobalMeta _ -> true
  | _ -> false
;;

let is_hasher cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | Hasher _ -> true
  | _ -> false
;;

let is_sivec cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | SInstrVec _ -> true
  | _ -> false
;;
...
let decls_of_type filter_f cid_decls = 
  let map_f (a, b) = match (filter_f cid_decls a) with 
    | true -> Some b
    | false -> None
  in 
...
let oids_of_type filter_f cid_decls = 
...
let is_acn_or_callable cid_decls oid = (is_acn cid_decls oid) || (is_callable cid_decls oid)
...
let tids_of_declmap cid_decls = 
  let tids = CL.filter_map (fun (id, _) -> if (is_tbl cid_decls id) then (Some id) else (None)) cid_decls in 
  tids
;;  

let mids_of_declmap cid_decls = 
  let mids = CL.filter_map (fun (id, _) -> if (is_meta cid_decls id) then (Some id) else (None)) cid_decls in 
  mids
;;
...
let stage_of_rid cid_decls rid = 
  ...
  | _ -> error ("stage_of_rid: identifier "^(P4tPrint.str_of_private_oid rid)^" does not reference a register")
;;
...
let stage_of_callable cid_decls tid = 
  ...
  | SchedBlock (_) -> None
  | _ -> error ("stage_of_callable: identifier "^(P4tPrint.str_of_private_oid tid)^" does not reference a callable (table or native block)")
;;
...
let print_nodepath (node_path:cid list) = 
  ...
  | [] -> ()
  | _ -> 
    printf "path from %s --> %s: " (P4tPrint.str_of_private_oid (Caml.List.hd node_path)) (P4tPrint.str_of_private_oid (Caml.List.hd (Caml.List.rev node_path))); 
    Caml.List.iter (fun i -> printf "%s, " (P4tPrint.str_of_private_oid i)) node_path; printf "\n"
;;
...
let rids_of_acn cid_decls acn_decl = 
  match acn_decl with 
  | Action (_, oids, _) ->  
    oids |> Caml.List.filter_map 
      (fun oid -> match (Cid.lookup cid_decls oid) with | SInstrVec(_, rid, _, _, _, _) -> Some rid | _ -> None) 
...
let set_stage_of_tid cid_decls tid new_stage = 
  ...
  match d with
    ...
    | _ -> error ("set_stage_of_tid: cannot set stage of "^(P4tPrint.str_of_private_oid tid)^" -- not a table")
;;
...
let stage_of_tid cid_decls tid = 
  ...
  match dec with 
  | Table (_, _, stage_opt) -> stage_opt
  | _ -> error ("stage_of_tid: identifier "^(P4tPrint.str_of_private_oid tid)^" does not reference a table")
;;
...
let succs_of_tid cid_decls tid = 
  ...
  let next_tids = Caml.List.fold_left (fun (acc:oid list) a -> acc@(succs_of_aid cid_decls a)) [] aids in 
...
let decls_of_tid cid_decls tid = 
  ...
  let oids = unique_list_of (CL.map (Cid.lookup cid_decls) (oids_of_tid cid_decls tid)) in 
  oids@acns
;;
...
let pred_aids_of_tid cid_decls tid = 
  ...
  let get_pred_opt tid d = match d with 
...
let rename_tid cid_decls (old_tid, new_tid) = 
  let change_tid decl_id old_tid new_tid = 
    ...
    | false -> decl_id
  in
  let cid_decls = CL.map (fun (i, d) -> (change_tid i old_tid new_tid, replace_oid_in_decl d old_tid new_tid)) cid_decls in 
  cid_decls 
;;
...
let insert_before_tid cid_decls newtbl_tid tid = 
  ...
  let newtbl_acns = acns_of_tid cid_decls newtbl_tid
    |> CL.map (add_succ_tid tid)
  in 
 ...
 *)
  let cid_decls = CL.fold_left emplace_decl cid_decls (newtbl_acns@updated_pred_acns) in 
  cid_decls 
;;
...
let insert_after_tid cid_decls tid newtbl_tid = 
  ...
  let newtbl_acns =  match nxt_tids with 
    ...
    | [] -> [] 
    | _ -> error "cannot place a table after tid -- tid has multiple next tables... "
  in 
  (* add newtbl_tid as the successor of all actions in tid *)
  let tbl_acns = match nxt_tids with 
    | [nt] -> acns_of_tid cid_decls tid |> CL.map (change_next_tid nt newtbl_tid) 
    | [] -> acns_of_tid cid_decls tid |> CL.map (add_succ_tid tid)
    | _ -> error "cannot place a table after tid -- tid has multiple next tables... "
  in 
  (* update cid_decls *)
  let cid_decls = CL.fold_left emplace_decl cid_decls (tbl_acns@newtbl_acns) in 
  cid_decls 
;;
...
let rec get_descendents decl_map oid : oid list = 
  ...
  let successor_tids = match dec with 
    | Table _ -> succs_of_tid decl_map oid
    | Action _ -> succs_of_aid decl_map oid
    | _ -> []
  in
  let successor_descendents = Caml.List.fold_left 
    (fun tid_acc oid -> tid_acc@(get_descendents decl_map oid)) [] successor_tids 
  in
  successor_tids@successor_descendents
;;

(* is tbl_id the descendent of action aid? *)
let is_descendent decl_map tbl_id aid = Caml.List.mem tbl_id (get_descendents decl_map aid)
...
let is_common_descendent decl_map tbl_id aids = 
  ...
  not (Caml.List.mem false bools) (* no falses --> it is a common descendent *)
;;

(***** action / table helpers *****)

let tid_of_aid decl_tups aid = 
  let f (tbl_id, d) = match d with
    | Table _ -> (
  ...
  match filtered_list with 
    | [(tid, _)] -> tid
    | _ -> error ("error finding calling table for action: "^P4tPrint.str_of_private_oid aid)
;;
...
let get_first_common_descendent decl_map acn_ids  = 
  ...
  match acn_ids with 
    | acn_id::acn_ids -> (
      ...
      let common_descendent_tids = Caml.List.filter (fun tid -> is_common_descendent decl_map tid acn_ids) descendent_tids in 
      match common_descendent_tids with 
      | [] -> None
      | first_descendent_tid::_ -> Some first_descendent_tid
    )
    | _ -> error "trying to find the common descendent of a table with 0 or 1 actions..."
;;
...
let to_callablepath cid_decls node_path = 
  ...
  let filter_f obj_id = match (Cid.lookup cid_decls obj_id) with 
    | Table _ -> true
    | SchedBlock _ -> true
    | _ -> false
  in 
...
let rec print_edgepath es = 
  ...
  | e::es -> (
    ...
    printf "[%s -> %s]" (P4tPrint.str_of_private_oid src_id) (P4tPrint.str_of_private_oid dst_id);
    print_edgepath es
  )
;;
...
let find_shortest_nodepath g root_id tgt_id = 
  ...
  let (edge_path, len) = 
    try D.shortest_path g root_id tgt_id 
    with Not_found -> ([], 0)
  in 
  let _ = len in 
...
let path_exists g cid_decls src_tid dst_tid = 
  let res = 
    match (((find_shortest_callablepath g cid_decls src_tid dst_tid) |> Caml.List.length)) with
...
let pred_tids_of g cid_decls tbl_id = 
  ...
  let pred_ids = CL.filter (fun src_id -> direct_path_exists g cid_decls src_id tbl_id) cids in
...
let succ_tids_of g cid_decls tbl_id = 
  let oids, _ = CL.split cid_decls in 
  let cids = CL.filter (is_callable cid_decls) oids  in 
  let succ_ids = CL.filter (fun dst_id -> direct_path_exists g cid_decls tbl_id dst_id) cids in
  succ_ids 
;;

let is_branch_tbl g cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | Table _ -> (
...
let is_merge_tbl g cid_decls oid = match (Cid.lookup cid_decls oid) with 
  | Table _ -> (
...
let tids_of_rid cid_decls reg_id = 
  let oids, _ = CL.split cid_decls in 
  let cids = CL.filter (is_callable cid_decls) oids  in 
  let tids = CL.filter (fun tbl_id -> tid_uses_rid cid_decls tbl_id reg_id) cids in 
  tids
;;

let sivec_uses_rid cid_decls sivec_id reg_id = 
  Cid.equals (rid_of_sivec (Cid.lookup cid_decls sivec_id)) reg_id
;;

(* find the stateful instructions that access reg id *)
let sivecs_of_rid cid_decls reg_id = 
  let oids, _ = CL.split cid_decls in 
  let sids = CL.filter (is_sivec cid_decls) oids  in 
  let filter_f sivec_id = sivec_uses_rid cid_decls sivec_id reg_id in 
  let sivec_ids = CL.filter filter_f sids in  
  sivec_ids 
;;
...
let lr_mids_of_tid cid_decls tbl_id : (lmid list * mid list) = 
  ...
  let lr_mids_of_obj obj_decl = 
    let v = 
      object
        ...
        method! visit_lmid _ m = 
          write_mids <- write_mids@[m];
        ...
        method! visit_mid _ m =
          read_mids <- read_mids@[m]
      end
    in
    v#visit_decl () obj_decl;
    (v#write_mids, v#read_mids)
  in 
  let (lmids_per_obj, rmids_per_obj) = CL.flatten objs 
    |> CL.map lr_mids_of_obj 
    |> CL.split 
  in 
  ...
  match (Cid.lookup cid_decls tbl_id) with
    ...
    | _ -> error ("lr_mids_of_tid: object "^(P4tPrint.str_of_private_oid tbl_id)^" not a table.")
;;
...
let is_call_tbl cid_decls tbl_id = match (Cid.lookup cid_decls tbl_id) with 
  | Table _ -> (
...
let is_join_tbl cid_decls tbl_id = match (pred_aids_of_tid cid_decls tbl_id) with 
  | [] -> false
  | _::[] -> false
  | _ -> true 
;;
...
let is_aid_used cid_decls aid = 
  ...
  let all_used_aids = CL.fold_left (fun aids tid -> aids@(aids_of_tid cid_decls tid)) [] tbl_ids in 
...
let table_dag_of (cid_decls:declsMap) : G.t = 
  ...
  let edge_fold_f g tid = 
    ...
    let g = CL.fold_left (fun g succ_tid -> G.add_edge g tid succ_tid) g succ_tids in 
    g
  in 
...
let table_g_of cid_decls g = 
  let edge_fold_f edges v = 
    let new_edges = 
      next_tables_of cid_decls g v
      |> CL.map (fun v_next -> (v, v_next))
    in 
    edges@new_edges
  in 
