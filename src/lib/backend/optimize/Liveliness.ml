...
let empty_varmap tg = G.fold_vertex (fun tid live_map -> (tid, [])::live_map) tg []
...
let rec live_analysis cid_decls g lm =  
	...
	match (is_converged) with 
		| true -> new_lm (* done *)
		| false -> live_analysis cid_decls g new_lm (* need another iteration *)
;;
...
let interf_g_of_dmap external_vars cid_decls = 
	let interf_g = 
		analyze_liveliness cid_decls
		|> interference_g_of_livemap cid_decls
	in 
	(* add conflict edges between all pairs of parameter variables *)
	let external_var_pairs = get_unique_unordered_pairs external_vars external_vars in 
	let fold_f g (s, t) = 
		match (s = t) with
			| false -> UG.add_edge g s t
			| true -> g (* don't add self edges *)
	in 
	let interf_g = CL.fold_left fold_f interf_g external_var_pairs in 
	dump_undirected interf_g (!BackendLogging.graphLogDir^"/interference_graph.dot");
	interf_g
;;
