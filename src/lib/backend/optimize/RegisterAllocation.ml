(* passes over the dag program.
  ...
  It basically transforms sivecs from salus to salu _calls_
...
let print_salu_input_conflict_hint rid vars conflict_pairs = 
  ...
  let hint_str = sprintf "[salu input merging] there are conflicts between the variables that the salus for register array %s access\n" (P4tPrint.str_of_private_oid rid) in 
  let hint_str = hint_str^(sprintf "[salu input merging] (%s) \n" (str_of_cids vars)) in 
  let map_f v = 
    match (Cid.lookup_opt (!var_contains_map) v) with 
    | None -> None
    | Some vs -> Some (sprintf "\t%s represents (%s)" (P4tPrint.str_of_private_oid v) (str_of_cids vs))
  in 
  let represented_vs_str = (CL.filter_map map_f vars |> (String.concat "\n"))^"\n" in 
  let hint_str = hint_str^represented_vs_str in 
  let hint_str = hint_str^(sprintf "conflict pairs: %s\n" (dbgstr_of_cidpairs conflict_pairs)) in 

  let hint_str = hint_str^(sprintf "[salu input merging] removing these conflicts may allow a more compact layout. \n") in 
...
let print_salu_input_merge_hint master_v (vs: mid list) = 
  ...
  let hint_str = sprintf "[salu input merging] %s <-- (%s)\n" (P4tPrint.str_of_private_oid master_v) (str_of_cids vs) in 
  !hintprint_endline hint_str
;;

let print_salu_merge_summary_hint () = 
  match (!unmergable_var_sets) with 
    | [] -> !hintprint_endline "note: all salu input / output variable were merged successfully! No call overhead."
    | var_sets -> !hintprint_endline (sprintf "note: %i var sets could not be merged. There is call overhead." (CL.length var_sets))
;;
...
let prepare_call_var_for_salu inner_var cid_decls (salu_call, outer_var) = 
  ...
  *)
  let tbl_id = Cid.concat inner_var (oid_of_sivec salu_call)in 
    ...
    object
      ...
      method! visit_mid _ mid = 
        match (Cid.equals mid v) with 
          | true -> 
            DBG.printf outc "[replace_var:renamer] in declaration: %s\n" (P4tPrint.str_of_private_oid (Option.get cur_oid));
            DBG.printf outc "[replace_var:renamer] replacing %s with %s\n" (P4tPrint.str_of_private_oid mid) (P4tPrint.str_of_private_oid new_v);
            new_v
          | false -> mid
...
let merge_vars_in arg_vars cid_decls vars = 
  ...
  let master_v = match arg_vars_in_vars with 
    | [] -> DBG.printf outc "[merge_vars_in] no arg vars found.\n"; CL.hd vars
    | [arg_var] -> DBG.printf outc "[merge_vars_in] 1 arg var found.\n"; arg_var
    | _ -> error "[merge_vars_in] error: cannot merge vars when multiple vars are args."
  in 
  let vars_to_remove = CL.filter (fun v -> not (Cid.equals v master_v)) vars in 
  DBG.printf outc "[merge_vars_in] merging (%s) --> %s\n" (str_of_cids vars_to_remove) (P4tPrint.str_of_private_oid master_v);
...
let cid_of_shared_arg_var sav = 
  match sav with | SharedIndex(cid) | SharedInput(cid) -> cid
;;

(* create and set an input variable for all salus that use a register *)
let prepare_frame_var_for_rid max_len unmergable_vars salu_param_finder prog_update_fcn (shared_salu_arg_var:shared_arg_var) cid_decls rid = 
  ...
  match ((CL.length vars) <= max_len) with 
    | true -> cid_decls (* nothing needs to change. *) (* nit -- technically, you can have 2. *)
    | false -> (
      ...
      match (CL.length conflict_pairs) with 
        ...
        | 0  -> (          
          DBG.printf outc "-------------\n";
          DBG.printf outc "[prepare_frame_var_for_rid] memory cell: %s\n" (P4tPrint.str_of_private_oid rid);
          DBG.printf outc "[prepare_frame_var_for_rid] parameter: %s\n" (P4tPrint.str_of_private_oid salu_arg_id);
        ...
        | _ -> (
          ...
          DBG.printf outc "[prepare_frame_var_for_rid] parameter: %s\n" (P4tPrint.str_of_private_oid salu_arg_id);
          DBG.printf outc "[prepare_frame_var_for_rid] vars: %s\n" (str_of_cids vars);
          DBG.printf outc "[prepare_frame_var_for_rid] conflict pairs: %s\n" (dbgstr_of_cidpairs conflict_pairs);
          DBG.printf outc "-------------\n";
          ...
          let var_size = match shared_salu_arg_var with 
            | SharedIndex _ -> 32
          ...
          let cid_decls = CL.fold_left (prog_update_fcn salu_arg_id) cid_decls salu_vars_map in 
          cid_decls          
        )
    )
;;
...
let salu_idx_vars_by_rid cid_decls rid = 
  let filter_map_f sivec_id = 
    let salu_call = Cid.lookup cid_decls sivec_id in 
    let idx_arg = idx_of_sivec salu_call in 
    match idx_arg with 
      | Const _ -> None
      | Meta m -> Some (salu_call, m)
  in 
  let var_idx_args = CL.filter_map filter_map_f (sivecs_of_rid cid_decls rid) in 
  var_idx_args
;;
...
let fst_salu_read_vars_by_rid cid_decls rid = 
  let filter_map_f sivec_id = 
    let salu_call = Cid.lookup cid_decls sivec_id in 
    let read_args = readargs_of_sivec salu_call in 
    match read_args with 
      | fst::_ -> (
        (* Printf.DBG.printf outc "[fst_salu_read_vars_by_rid] rid: %s read_var: %s\n" (mid_to_str_suffix rid) (mid_to_str_suffix fst); *)
        Some (salu_call, fst)
      )
      | _ -> None
  in 
  let pos_args = CL.filter_map filter_map_f (sivecs_of_rid cid_decls rid) in 
  pos_args
;;
...
let salu_ret_vars_by_rid cid_decls rid = 
  let filter_map_f sivec_id = 
    let salu_call = Cid.lookup cid_decls sivec_id in 
    let ret_arg = outarg_of_sivec salu_call in 
    match ret_arg with
      | Some ret_mid -> Some (salu_call, ret_mid)
      | None -> None 
  in 
  let out_args = CL.filter_map filter_map_f (sivecs_of_rid cid_decls rid) in 
  out_args
;;
...
let prepare_salu_frames_for_rid unmergable_vars cid_decls rid = 
  (* merge index vars *)  
  let cid_decls = prepare_frame_var_for_rid 
    1
    unmergable_vars
    salu_idx_vars_by_rid (* how to find index vars *)
    prepare_call_var_for_salu(* what code to add to the program to prepare the call *)
  ...
  let cid_decls = prepare_frame_var_for_rid 
    ...
    prepare_call_var_for_salu (* what code to add to the program to prepare the call *)
    (SharedInput(shared_reg_argid "ARG1_VAR_OF_" rid)) (* name of the first positional var used by this register's salu *)
    cid_decls 
    rid 
  in 
  let cid_decls = prepare_frame_var_for_rid 
    ...
    prepare_call_var_for_salu (* what code to add to the program to prepare the call *)
    (SharedInput(shared_reg_argid "ARG2_VAR_OF_" rid)) (* name of the second positional var used by this register's salu *)
    cid_decls 
    rid 
  in 
  (* merge output vars  *)
...
let find_unbound_vars cid_decls = 
  ...
  let mids_of_compute_objs dec = 
    ...
    v#mids
  in 
  let is_compute_object d = match d with 
    | GlobalMeta _
    | StructDef _ 
    | Struct _ 
    | DConst _
  ...
  let all_mids = 
    CL.map mids_of_compute_objs compute_ds |> 
    CL.flatten 
  in 

  let mid_of_metadec d = match d with 
    | GlobalMeta(mid, _) -> Some mid
    | _ -> None
  in 
...
let prepare_salu_frames dag = 
  ...
  let rids = oids_of_type is_reg cid_decls in 
  let updated_cid_decls = CL.fold_left (prepare_salu_frames_for_rid struct_mids) cid_decls rids in 
...
let doPasses dag =
  ...
  let framed_dag = prepare_salu_frames dag in 
  framed_dag
  (* dag *)
;;
