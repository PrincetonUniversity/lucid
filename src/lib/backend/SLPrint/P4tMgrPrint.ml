...
|__pysrc__}
  ...
  let str_of_mc_node_tup mc_instr =
    sprintf "(%i, %i)" mc_instr.port mc_instr.pkt_copy_id
  ;;

  let str_of_mc_nodes mc_instrs =
    sprintf "[%s]" (String.concat ", " (CL.map str_of_mc_node_tup mc_instrs))
  ;;

  let str_of_mc_group mc_group =
    sprintf
      "m.add_multinode_mc_group(%i, %s)"
      mc_group.mc_id
