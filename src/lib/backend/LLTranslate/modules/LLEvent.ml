...
let event_visible_args_instrs hdl_id ev_rec ev_args =
  ...
  Generators.oper_assign_instrs visible_ev_fields rhs_exps
;;
...
let event_hidden_args_instrs ev_rec =
  let hidden_ev_fields =
    CL.map
      (fun fld -> Cid.concat ev_rec.event_struct_instance fld)
      (ev_rec.hidden_fields |> CL.split |> fst)
  in
  let rhs_exps = CL.map Generators.int_expr [ev_rec.event_iid; 0; 0] in
  Generators.oper_assign_instrs hidden_ev_fields rhs_exps
;;
...
let event_meta_init_instrs evrec =
  event_hidden_args_instrs evrec @ event_other_setup_instrs evrec
;;
...
let event_combinator_inliners = [event_delay_cid, inlined_event_delay]
...
let generate_self (args : codegenInput) : codegenOutput =
  ...
  | ECall (ev_id, ev_args) ->
    ...
    let oid, obj =
      ...
      | false -> from_event_call hdl_id (Option.get args.basename) ev_id ev_args
    in
    { names = [oid]; objs = [obj] }
  | EVar _ ->
    error
      "backend does not support generating previously declared events-- all \
       events must be declared inline."
    (* { names = []; objs = [] }     *)
...
let generate_port (args : codegenInput) : codegenOutput =
  ...
  let _ = port in
  let ev_rec, ev_args =
    match event_exp.e with
    | ECall (ev_cid, ev_args) ->
      let ev_rec =
        match ctx_find_eventrec_opt ev_cid with
        | Some ev_rec -> ev_rec
        | None ->
          error
            ("[generate_port] could not find declaration of event: "
            ^ Cid.to_string ev_cid)
      in
      ev_rec, ev_args
    | EVar _ -> error "[generate_port] event variables not supported"
    | _ -> error "[generate_port] unexpected expression formfor event argument."
  in
  (* the vector of instructions to
     set fields for this generate call. *)
  let ivec = event_visible_args_instrs hdl_id ev_rec ev_args in
  (* event_meta_instrs @ ivec @ runtime_instrs in *)
  (* return a declaration of an alu with this vector of instructions *)
  let alu_id =
    Cid.compound (Id.create "generate_alu") (Option.get args.basename)
  in
  let alu_obj = IS.new_dinstr alu_id ivec in
