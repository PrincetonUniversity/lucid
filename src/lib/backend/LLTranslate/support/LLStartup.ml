...
let event_pkt_generator_mc_config =
  let mk_mc_group num_extra_copies =  
    ...
    in
    let copy_instrs = CL.map mk_mc_copy_instr (range 1 (num_extra_copies + 2)) in 
    { 
      mc_id = lucid_mc_group+num_extra_copies; (* base group represents 1 copy. *)
  ...
  new_config_block
    (Cid.create ["event_pkt_generator"])
    (McConfig mc_groups)
