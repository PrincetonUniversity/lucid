...
let str_of_varids mids = 
  CL.map str_of_varid mids |> 
  String.concat ~sep:", " 
;;
...
let rec str_of_private_oid (m:mid) : string =
  ...
  match (Caml.List.rev id_list) with 
    ...
    | [(last_n, last_i)] -> (
      "dpt"^delim^(string_of_int last_i)^delim^last_n
    )
    | (last_n, last_i)::front_rev -> (
      ...
      "dpt"^delim^(string_of_int last_i)^delim^(Caml.String.concat delim str_list)^delim^last_n
    )
;;
...
module PrimitiveString = struct 
  ...
  let sized_str_of_oper oper =
    match oper with
    | Meta m -> str_of_varid m
    | Const c -> (string_of_int (Integer.to_int c))^"w"^(string_of_int (Integer.size c))
  ;;
  ...
  let str_of_pat pat =
    let pat_strs = match (CL.length pat) with 
    | 0 -> str_of_cond Any (* this is the equivalent of a default rule. *)
    | 1 -> str_of_cond (snd (CL.hd pat))
    | _ -> 
      let field_strs = Caml.List.map (fun (_, cond) -> str_of_cond cond) pat in
...
module PrintSalu = struct 
  ...
  let str_of_soper sv : string = 
    match sv with 
      | RegVar _ -> memName
      | MetaVar m -> str_of_varid m
      | ConstVar c -> PrimitiveString.str_of_const c
      | NoOper -> ""

  ;;
  let str_of_sbinop op rs oper : string =
    sprintf "%s %s %s" (str_of_soper rs) (PrimitiveString.str_of_binop op) (str_of_soper oper)
  ;;
  let string_of_sexpr sExpr =
    match sExpr with
    | SVar v -> str_of_soper v
  ...
  let print_spred_instr fmt (spred_expr : sPredExpr) =
    ...
    | (Comp (sv1, bin_op, sv2, cmp_op, sv3)) ->
      let sv1 = str_of_soper sv1 in 
      let sv2 = str_of_soper sv2 in 
      let sv3 = str_of_soper sv3 in 
      fprintf fmt "if (%s %s %s %s %s){@," sv1 (PrimitiveString.str_of_binop bin_op) sv2 (PrimitiveString.str_of_cmpop cmp_op) sv3
  ...
  let print_sinstrs (statefulIv : sInstrVec) rid_width =
    ...
    match (has_ret_istr statefulIv) with 
      | true ->   fprintf str_formatter "bit<%s> %s;@," (string_of_int rid_width) tmpName; 
      | false -> ();
    );
  ...
  let print_sivec fmt callable_id rid rid_width siv out_var_opt idx_oper =
    (* print_endline ("printing sivec: " ^ qstr_of_mid callable_id); *)
...
module PrintTable = struct 
  ...
  let print_tbl_default fmt rules = 
    let is_wildcard r = 
      ...
      | Match (_, [], _) -> true (* if there's no pattern at all, its a wildcard. *)
      | _ -> false
    in 
    let wildcard_rules = CL.filter is_wildcard rules in (* this is a default rule. *)
    let rules = CL.filter (fun r -> not (is_wildcard r)) rules in       
    match wildcard_rules with 
      | [] -> fprintf fmt "//no default action@,"; rules
      | [r] -> let _ = r in (* fprintf fmt "const default_action = %s();@," (str_of_private_oid (aid_of_rule r)); *) rules
      | _ -> error "[print_tbl_rules] more than 1 wildcard rule in single-column table."
  ;;
  ...
  let print_tbl_pragmas fmt tbl_id stage_opt stage_tbl_map =
    match stage_opt with
      | Some stage_num -> (
        ...
        let other_ss_tbls = CL.filter (fun t -> not (Cid.equals tbl_id t)) same_stage_tbls in 
...
module PrintComputeObject = struct 
  ...
  let print_var fmt midWidth midId = 
    (* print_var_pragmas fmt newMid; *) (* allow P4 to overlay variables by commenting out this line *)
  ...
  let print_decl stage_tbl_map fmt (decl : decl) =
    match decl with
    | RegVec (rid, wid, len, def, _) -> 
      print_reg fmt rid wid len def
    | InstrVec (iid, iVec) -> 
      print_ivec fmt iid iVec
    | SInstrVec (sid, rid, rid_width, siv, out_var_opt, idx_oper) ->
      PrintSalu.print_sivec fmt sid rid rid_width siv out_var_opt idx_oper
    ...
    | GlobalMeta (newMid, midWidth) -> 
      print_var fmt midWidth newMid
    | Action (actn_id, obj_ids, next_tids) -> 
      print_action fmt actn_id obj_ids next_tids;
    | Table (tbl_id, branches, stage_opt) ->
      PrintTable.print_tbl fmt tbl_id (match_vars_of_rules branches) branches stage_opt stage_tbl_map 
    | SchedBlock(_, _) -> ()
  ...
  let filter_decl decl = 
    match decl with 
      | RegVec _ | InstrVec _ | SInstrVec _ | Hasher _ 
      | GlobalMeta _ | Action _ | Table _ | SchedBlock _  -> true 
      | _ -> false
  ;;
  let print_decls decls = 
    ...
    print_fmt_list str_formatter printable_decls (print_decl (stageMap_of_tbls decls))
  ;;
end
...
module PrintStruct = struct 
  ...
  let print_inst fmt decl = 
    match decl with 
      | Struct(s_inst_name, _, s_def_name) ->
        fprintf fmt "%s %s;" (str_of_public_globalid s_def_name) (strbase_of_varid s_inst_name) 
...
module PrintParseTree = struct 
  ...
  let string_of_select_pat s_pat = 
    match s_pat with 
      ...
      | SConstVar mid -> print_endline ("[string_of_select_pat]"^(show_select_pat s_pat)); str_of_public_globalid mid 
      | SDefault -> "default"
  ;; 
...
let print_p4_struct_defs decls = 
  let struct_decls = 
    CL.filter (fun d -> match d with StructDef _ -> true | _ -> false)
    decls 
  in 
  open_block ();
  fprintf str_formatter "@,";
...
let print_p4_const_defs decls = 
  ...
  let constdef_decls = 
    CL.filter_map 
    (fun d -> match d with DConst(name, scope, width, valu) -> Some (name, scope, width, valu)  | _ -> None)
    decls 
  in   
...
(* format all struct definitions of type HeaderStruct *)
let print_p4_hdr_insts decls = 
  let filter_f dec = 
    match dec with
      | Struct(_, _, s_def_name) -> (
...
let print_p4_parser decls = 
  ...
  let filter_f dec = 
    match dec with 
      | ParseTree(parser_name, nodes) ->
        Some (parser_name, nodes)
      | _ -> None
  in
...
let print_p4_prototype_egr () = 
  (* this only supports one event generation per input packet and only 
    ...
    let egr_apply_body_str = indent_block ~nspaces:8 "\n\
    ...
    \   } // packets not on the DPT port and not DPT_ETYPE are regular packets, do nothing.\n\
    \ } else {\n\
    \   if (hdr.ethernet.ether_type != DPT_ETYPE) {\n\
    \     // this is a non dpt packet on the DPT recirc port -- should not happen.\n\
    \     eg_dprsr_md.drop_ctl = 1;\n\
    \   }\n\
    }\n\
    "
...
let print_located_sched_block tgt_loc d = 
  match d with 
    | SchedBlock(oid, {sb_print=sb_print; sb_loc=sb_loc;}) -> (
...
let print_located_sched_call tgt_loc d = 
  match d with 
    | SchedBlock(oid, {sb_print=_; sb_loc=sb_loc;}) -> (
        match (sb_loc == tgt_loc) with 
        | true -> 
          fprintf str_formatter "%s.apply();" (str_of_private_oid oid)
        | false -> ()
    )
    | _ -> ()
;;
...
let from_straightline (blk:tblSeqProg) = 
  ...
  let dpt_igr_obj_str =
     (str_of_igr_objs blk.tspdecls)
    ^(str_of_igr_scheds blk.tspdecls)
  in 

  let dpt_igr_call_str = 
    (print_p4_calls blk.tsptblseq) 
    ^(callstr_of_igr_scheds blk.tspdecls)
  in 
  let dpt_hdr_decls_str = 
      print_p4_const_defs blk.tspdecls (* defines go in the same block as struct defs *)
