...
let ctx_add_structdef decl = 
  match decl with 
    | StructDef(_) -> p4tCtx := P4tCtx.add 
      (name_of_structdef decl)
      decl
      (!p4tCtx)
    (* also add const defs, for the final emit table. *)
    | DConst(var_id, _, _, _) ->   
      print_endline ("[ctx_add_structdef] adding const def: "^(Cid.to_string var_id));
      p4tCtx := P4tCtx.add var_id decl (!p4tCtx)
    | _ -> ()
;;

let ctx_add_structdefs decls = 
  CL.iter ctx_add_structdef decls
;;
...
let ctx_get_event_iid event_id = 
  ...
  match (P4tCtx.find event_enum_var_id !p4tCtx) with 
    ...
    | _ -> error "[ctx_get_event_iid] unexpected declaration when looking for event enum."
;;

let ctx_add_structinst decl = 
  match decl with 
    | Struct(_) -> p4tCtx := P4tCtx.add 
      (name_of_structinst decl)
      decl
      (!p4tCtx)
    | _ -> ()
;;

let ctx_add_structinsts decls = 
  CL.iter ctx_add_structinst decls
;;