...
let inlined_event_delay alu_obj args =
  let oid, ivec = unpack_dinstr alu_obj in 
  let delay = new_eoper (oper_from_immediate (CL.hd args)) in 
  match ivec with 
...
let inlined_event_sslocate alu_obj args =
  let oid, ivec = unpack_dinstr alu_obj in 
  let loc = new_eoper (oper_from_immediate (CL.hd args)) in 
  match ivec with 
  | id_set::mc_set::loc_set::delay_set::fields_set -> 
...
let inlined_event_smlocate alu_obj args =
  let oid, ivec = unpack_dinstr alu_obj in 
  let loc = new_eoper (oper_from_immediate (CL.hd args)) in 
...
let rec generate_modified_event alu_name fcn_id fcn_args = 
  ...
  | true -> (
    ...
    let alu_id, alu_obj = generate_modified_event 
      alu_name 
      inner_fcn_id
      inner_args
    in 
    ...
    let updated_alu_obj = inliner_f alu_obj (CL.tl fcn_args) in 
    alu_id, updated_alu_obj
  )
  | false -> TofinoAlu.from_event_instantiation alu_name fcn_id fcn_args
;;
...
let generate_event (args : codegenInput) : codegenOutput = 
  match (CL.hd args.args).e with 
    | ECall(ev_id, ev_args) -> (
        ...
        *)
        let oid, obj = match (Cid.exists event_combinator_inliners ev_id) with 
          | true -> 
            generate_modified_event (Option.get args.basename) ev_id ev_args
          | false ->
            TofinoAlu.from_event_instantiation (Option.get args.basename) ev_id ev_args
        in         
