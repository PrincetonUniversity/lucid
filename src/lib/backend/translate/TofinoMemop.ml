...
let from_testexp mem_var (exp:S.exp) = 
      ...
      | S.EOp(rel_op, [lhs; rhs]) -> (
        match (lhs, rhs) with 
          ...
          | ({e=S.EOp(arith_op, [e1; e2]); _}, e3) -> 
            ...
            let o1 = soper_from_immediate (Some mem_var) e1 in 
            let o2 = soper_from_immediate (Some mem_var) e2 in 
            let o3 = soper_from_immediate (Some mem_var) e3 in 
            Comp(o1, arith_op, o2, rel_op, o3),
            Neg(o1, arith_op, o2, rel_op, o3)
          (* a < (b + c) *)
          ...
          | (e1, e2) -> 
            ...
            let arith_op = Add in 
            let o1 = soper_from_immediate (Some mem_var) e1 in 
            let o2 = ConstVar(Integer.of_int 0) in 
            let o3 = soper_from_immediate (Some mem_var) e2 in 
            Comp(o1, arith_op, o2, rel_op, o3),
            Neg(o1, arith_op, o2, rel_op, o3)
      )
      | _ ->
        error "[tofino memop translator] memop test expression does not contain a relational operation at its root."
;;

let from_retexp mem_var (exp:S.exp) = 
  (* the backend syntax only currently supports expressions that are immediates or have 1 binary operation. *)
   match exp.e with 
    ...
    (* a *)
    | _ -> SVar(soper_from_immediate (Some mem_var) exp)
;;
...
let from_memop 
  ...
  : sInstr list
  = 
  let mem_var, arg_var = match args with 
  ...
  let sinstr_tups = match bdy.s with 
    (* evaluate expression *)
    | S.SRet (Some exp) -> [(None, from_retexp mem_var exp)]
    (* test-then-evaluate expression *)
    | S.SIf(test_exp, {s=SRet(Some rexp1); _}, {s=SRet(Some rexp2); _}) -> (
      let true_test, false_test = from_testexp mem_var test_exp in 
      let true_exp = from_retexp mem_var rexp1 in 
      let false_exp = from_retexp mem_var rexp2 in      
      [(Some true_test, true_exp); (Some false_test, false_exp)]
    )
    | _ -> error "invalid form for a memop"
  in 
  let sinstrs = match kind with 
    | Get -> CL.map (fun (a, b) -> RetInstr(a, b)) sinstr_tups
    | Set -> CL.map (fun (a, b) -> MemInstr(a, b)) sinstr_tups
  in 
  sinstrs
;;
...
let from_memop_name 
  ...
  from_memop param_cids body_stmt kind in_var
;;
