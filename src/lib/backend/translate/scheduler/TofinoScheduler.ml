...
(* utils *)
let struct_name   evid = P4tPrint.str_of_varid (TofinoStructs.in_struct_from_ev evid)
let instance_name evid = P4tPrint.str_of_varid (TofinoStructs.full_in_struct_from_ev evid EBackground)
;;

let hex_iid evid = 
  sprintf "%#x" (ctx_find_eventrec (Cid.id evid)).event_iid
;;
...
type cmd = string
type acn = {aname:string; acmds:cmd list; arule: string}
type tbl = {tid:Cid.t; tacns:acn list; tkeys: string list;}
(* table has an id for integration into the syntax tree *)

let cmd_lucid_etype = sprintf "hdr.ethernet.ether_type = %#x;" lucid_etype;;
let cmd_ip_etype = sprintf "hdr.ethernet.ether_type = %#x;" ip_etype;;
...
module IngressExit = struct 
  ...
  let acn_stop bg_ev_ids = 
    ...
    {
      aname="acn_stop";
      acmds=cmd_exit::cmd_drop::[];
      arule="(0x0, 0x0) : acn_stop();"
    }
  ;;
  (* return (an exit event was generated) *)
  ...
  let acn_return bg_ev_ids = 
    {
      ...
      arule="(0x0, 0x0) : acn_return();"
    }
  ;;
  (* bg (a single bg event was generated) *)
  (* we technically don't need to do a copy -- the data plane packet could 
    just become an event packet. But doing the copy simplifies the 
    egress logic *) 
  let acn_bg bg_ev_ids ev_id = 
    let acn_name = "acn_bg_"^(struct_name ev_id) in 
    {
      aname=acn_name;
      acmds=cmd_copy_to_recirc::cmd_exit::cmd_lucid_etype::(cmd_disable_other_hdrs bg_ev_ids ev_id);
      arule="(0x0, "^(hex_iid ev_id)^") :" ^ acn_name ^"();"
    }
  ;;

  (* bg and return (one bg event and one exit event was generated) *)
  let acn_bg_ret bg_ev_ids ev_id = 
    let acn_name = "acn_bg_ret"^(struct_name ev_id) in 
    {
      aname=acn_name;
      acmds=cmd_copy_to_recirc::cmd_lucid_etype::(cmd_disable_other_hdrs bg_ev_ids ev_id);
  ...
  (*** printers ***)
  let tbl_acns_str tbl = 
    CL.map acn_str tbl.tacns
    |> String.concat "\n"
  ;;
