      ...
      (fun i e' ->
        ...
        then
          ...
          (* WARNING: This is a hack that needs to be fixed if users can write tuples. 
              ...
              into tuples. *)
          let expected_eff = match (e'.e) with 
            | EOp(TGet(_, idx), _) -> 
              wrap_effect tuple_eff [None, 0; None, idx]
            | _ -> 
              wrap_effect tuple_eff [None, 0; None, i] 
          in
          let expected = {(fresh_type ()) with 
            teffect = expected_eff}
          in
          unify_ty e.espan expected (Option.get e'.ety)
        )
      )
