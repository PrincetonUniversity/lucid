  ...
  | Some next_event_time -> (
    ...
    let nst = if (idx = 0) 
      then (
        ...
        run_egress_events Config.cfg.show_interp_events nst;
        nst)
      else nst
    in
    if nst.current_time > nst.config.max_time
      then nst
        ...
        | Some (epgs) -> execute_interp_event Config.cfg.show_interp_events execute_sim_step idx nst epgs
...
let simulate (nst : State.network_state) =
  if (not Config.cfg.json) && not Config.cfg.interactive
  then
    Console.report
    @@ "Using random seed: "
  ...
  finish_egress_events Config.cfg.show_interp_events nst;
  nst
;;
...
let rec execute_interactive_sim_step event_getter_opt max_time idx nst = 
  ...
  | Some t -> 
    ...
    if max_time > -1 && nst.current_time > max_time
      ...
      | Some (epgs) ->
        execute_ready_controls idx nst;
        execute_interp_event Config.cfg.show_interp_events next_step_continuation idx nst epgs
