...
(* event match elimination, for use in tofino backend
    - copied from SpecRegex branch on 11/30/23
    - untested and might need some cleanup
*)
...
let get_map var_infos event_infos match_exp = 
  ...
  | EVar (cid) -> (match IdMap.find_opt (Cid.to_id cid) var_infos with
                    ...
                    | None -> None)
  | ECall (cid, exps, _) -> (match IdMap.find_opt (Cid.to_id cid) event_infos with 
...
let replace_exp var_infos event_infos exp = 
  ...
  | EVar (cid) -> (match IdMap.find_opt (Cid.to_id cid) var_infos with
                    ...
                    | None -> exp)
  | ECall (cid, params, _) -> (match IdMap.find_opt (Cid.to_id cid) event_infos with 
...
let replacer = 
  object (self) 
    ...
    method! visit_SAssign env id exp = 
      match (IdMap.find_opt (Cid.to_id id) (!env).var_infos) with
      | Some vi -> print_endline ("REPLACED LOCAL ASSIGN"); (match exp.e with
                    | ECall (eid, args, _) ->
                      let ev_info = (IdMap.find (Cid.to_id eid) (!env).event_infos) in
                      let ev_int_asgn = sassign (Cid.id vi.ev_id_int) (make_num_size ev_info.i 8) in
                      let param_asgns = List.map2 (fun (id,_) exp -> sassign (Cid.id id) exp) (IdMap.find (Cid.to_id eid) vi.ev_params_map) args in
