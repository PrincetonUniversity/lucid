(* DEPRECATED AS OF 4/23/23, see immutableConditions.ml instead *)
...
let rec refresh_cid cid =
  ...
  | Cid.Id id ->
    let unique_name =
      fst id ^ "_branch_test_copy_" ^ (snd (Id.refresh id) |> string_of_int)
    in
...
let assigned_in_one_stmt evid_params query_id stmts =
  CL.map (assigned_in_stmt evid_params query_id) stmts
  |> CL.fold_left ( || ) false
;;
...
let update_branch_exp evid_params stmts exp =
  let new_exp, new_var_decls =
    CL.fold_left
      (update_exp_for_one_var evid_params stmts)
      (exp, [])
      (evars_in_exp exp)
  in
    ...
    object
      ...
      method! visit_statement ctx statement =
        ...
        Also generate the statement that sets the constant copy. *)
          let new_exp, new_var_decls =
            update_branch_exp evid_params [st1; st2] exp
          in
