...
events mk_ethpkt(uint64_t dst_752 , uint64_t src_753 , uint16_t ety_754 ){
  events tmp_802  = {.meta = {.len = 14, .is_packet = 1, .has_payload = 1, .timestamp = 0}, .data = ethpkt_756(dst_752, src_753, ety_754)};
  return tmp_802;
}
...
uint16_t handle_event(uint32_t ingress_port , events*  ev_in , out_event out_events [64]){
  ...
  switch (ev_in->data.tag) {
    case 1: {
      ...
      if (ety_759 == 2048) {
        out_event tmp_803  = {.ev = mk_ethpkt(src_758, dst_757, ety_759), .out_loc = 2, .port = ingress_port};
