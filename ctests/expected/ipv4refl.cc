...
event_variant ethpkt_756(uint64_t dst_752 , uint64_t src_753 , uint16_t ety_754 ){
  event_variant ev  = {0};
  ev.payload.ethpkt_756.dst_752 = dst_752;
  ev.payload.ethpkt_756.src_753 = src_753;
  ev.payload.ethpkt_756.ety_754 = ety_754;
  ev.tag = ethpkt_tag;
  return ev;
}
typedef struct {
...
events mk_ethpkt(uint64_t dst_752 , uint64_t src_753 , uint16_t ety_754 ){
  events tmp_802  = {.meta = {.len = 14, .is_packet = 1, .has_payload = 1, .timestamp = 0, .in_port = 0}, .data = ethpkt_756(dst_752, src_753, ety_754)};
  return tmp_802;
}
...
uint16_t handle_event(events*  ev_in , out_event out_events [64]){
  ...
  switch (ev_in->data.tag) {
    case 1: {
      uint64_t dst_757  = ev_in->data.payload.ethpkt_756.dst_752;
      uint64_t src_758  = ev_in->data.payload.ethpkt_756.src_753;
      uint16_t ety_759  = ev_in->data.payload.ethpkt_756.ety_754;
      events this  = mk_ethpkt(dst_757, src_758, ety_759);
      if (ety_759 == 2048) {
        out_event tmp_803  = {.ev = mk_ethpkt(src_758, dst_757, ety_759), .out_loc = 2, .port = ev_in->meta.in_port};
...
void deparse_event(events*  ev_out , packet_t*  buf_out ){
  switch (ev_out->data.tag) {
    case 1: {
      uint64_t dst_752  = ev_out->data.payload.ethpkt_756.dst_752;
      uint64_t src_753  = ev_out->data.payload.ethpkt_756.src_753;
      uint16_t ety_754  = ev_out->data.payload.ethpkt_756.ety_754;
