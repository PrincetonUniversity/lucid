...
event_variant tick_670(){
  event_variant ev  = {0};
  
  ev.tag = tick_tag;
  return ev;
}
event_variant pkt_in_672(uint32_t x_671 ){
  event_variant ev  = {0};
  ev.payload.pkt_in_672.x_671 = x_671;
  ev.tag = pkt_in_tag;
  return ev;
}
event_variant pkt_out_674(uint32_t x_673 ){
  event_variant ev  = {0};
  ev.payload.pkt_out_674.x_673 = x_673;
  ev.tag = pkt_out_tag;
  return ev;
}
typedef struct {
...
uint16_t handle_event(events*  ev_in , out_event out_events [64]){
  ...
  switch (ev_in->data.tag) {
    case 3: {
      uint32_t x_675  = ev_in->data.payload.pkt_out_674.x_673;
      events this  = mk_pkt_out(x_675);
      break;
    }
    case 1: {
      
      events this  = mk_tick();
      out_event tmp_719  = {.ev = mk_tick(), .out_loc = 1, .port = 0};
      out_events[n] = tmp_719;
      n = n + 1;
      break;
    }
    case 2: {
      uint32_t x_676  = ev_in->data.payload.pkt_in_672.x_671;
      events this  = mk_pkt_in(x_676);
      out_event tmp_720  = {.ev = mk_pkt_out(x_676), .out_loc = 2, .port = 0};
...
void deparse_event(events*  ev_out , packet_t*  buf_out ){
  switch (ev_out->data.tag) {
    ...
    case 2: {
      uint32_t x_671  = ev_out->data.payload.pkt_in_672.x_671;
    ...
    case 3: {
      uint32_t x_673  = ev_out->data.payload.pkt_out_674.x_673;
