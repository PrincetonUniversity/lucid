...
typedef struct {
  ...
  union {
    struct {
      uint32_t src_ip_1184;
      uint32_t src_port_1185;
    } pkt_in_1186;
    struct {
      uint32_t src_port_1187;
    } bg_cmd_1188;
    struct {
...
events mk_pkt_in(uint32_t src_ip_1184 , uint32_t src_port_1185 ){
  events tmp_1270  = {.meta = {.len = 8, .is_packet = 0, .has_payload = 0, .timestamp = 0}, .data = pkt_in_1186(src_ip_1184, src_port_1185)};
  return tmp_1270;
}
events mk_bg_cmd(uint32_t src_port_1187 ){
  events tmp_1271  = {.meta = {.len = 4, .is_packet = 0, .has_payload = 0, .timestamp = 0}, .data = bg_cmd_1188(src_port_1187)};
  return tmp_1271;
}
events mk_pkt_out(uint32_t src_ip_1189 , uint32_t src_port_1190 ){
  events tmp_1272  = {.meta = {.len = 8, .is_packet = 0, .has_payload = 0, .timestamp = 0}, .data = pkt_out_1191(src_ip_1189, src_port_1190)};
  return tmp_1272;
}
...
uint8_t parse_event(packet_t*  packet , events*  next_event ){
  ...
  switch (ety) {
    case 666: {
      ...
      switch (tag) {
        case 1: {
          uint32_t src_ip_1184  = ((uint32_t)(read_bits(packet, 32)));
          uint32_t src_port_1185  = ((uint32_t)(read_bits(packet, 32)));
          (*(next_event)) = mk_pkt_in(src_ip_1184, src_port_1185);
          return packet->cursor <= packet->end;
          break;
        }
        case 2: {
          uint32_t src_port_1187  = ((uint32_t)(read_bits(packet, 32)));
          (*(next_event)) = mk_bg_cmd(src_port_1187);
          return packet->cursor <= packet->end;
          break;
        }
        case 3: {
          uint32_t src_ip_1189  = ((uint32_t)(read_bits(packet, 32)));
          uint32_t src_port_1190  = ((uint32_t)(read_bits(packet, 32)));
          (*(next_event)) = mk_pkt_out(src_ip_1189, src_port_1190);
          return packet->cursor <= packet->end;
          break;
        }
        default: {
